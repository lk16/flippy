...
// priorityEntry is the value stored in the priority queue for one pending evaluation job.
...
// will request the next level once the current job completes.
func (s *Server) enqueuePriority(ctx context.Context, board string, level int) error {
	...
	data, err := json.Marshal(priorityEntry{Board: board, Level: level})
	if err != nil {
